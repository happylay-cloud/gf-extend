...
func NewZinxLog(out io.Writer, prefix string, flag int) *ZinxLogger {
	...
	// 默认debug打开，calledDepth深度为2，ZinxLogger对象调用日志打印方法最多调用两层到达output函数
	zlog := &ZinxLogger{out: out, prefix: prefix, flag: flag, file: nil, debugClose: false, calldDepth: 2}
	// 设置log对象，回收资源析构方法（不设置也可以，go的Gc会自动回收）
		...
		if log.flag&(BitShortFile|BitLongFile) != 0 {
			...
			if log.flag&BitShortFile != 0 {
				...
				for i := len(file) - 1; i > 0; i-- {
					if file[i] == '/' {
						// 找到最后一个'/'之后的文件名称  如:/home/go/src/gf_plus.go 得到"gf_plus.go"
						short = file[i+1:]
						break
					}
				}
...
// itoa 将一个整形转换成一个固定长度的字符串，字符串宽度应该是大于0的，要确保buffer是有容量空间的
